...
2. 软件会根据 Windows、macOS Intel 或 macOS Apple Silicon 自动选择安装包。
3. 国内网络优先点击“加速下载”；镜像不可用时切换“GitHub 原地址”。
4. 下载前可核对安装包大小和 GitHub 返回的 SHA-256。
5. 下载完成后先退出软件，再运行安装包覆盖安装。
6. 商品、历史价格、SKU 监控价、账号浏览器目录和飞书配置保存在用户数据目录，覆盖安装不会删除。
