...
1. 打开“运营数据 > 数据仓库 > 报表上传”，先选择店铺和报表周期（日、周、月或自定义），再选择文件。
2. 系统会预览报表类型、统计日期、字段和有效行数。自动识别仅作为初始值；周报、月报或文件名日期不等于统计日期时，必须在确认前手工修正。
3. 导入后按“统计日期”查看文件；可勾选整组日期，也可展开后只选择某一份报表，再批量删除。
4. 商品资料在“数据仓库 > 商品资料”维护。型号和品类按“店铺 + 商品 ID”分别保存；可下载模板、导入表格或手工新增，重复记录会被拦截。

### 查看经营数据

- “整店总览”查看净 GSV、推广花费、管理 ROI 和推广费率；点击指标卡可单选或多选经营趋势。“类目销售贡献与推广效率”会把类目净 GSV、销售占比、推广花费、花费占比和推广费率放在同一行，便于老板直接比较。
...
- 日期快捷项包括今天、昨天、上周、本月、上月和自定义。自定义起止日期选择完成后点击“确定”，四个运营页面会一起刷新。
