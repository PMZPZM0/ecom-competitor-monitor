...
4. 商品标题、店铺和型号由抓取结果自动识别。
5. 选择参与采价的账号类型；需要 750 主图、详情图或视频时勾选“抓取完整素材”，需要买家秀时再勾选“同时抓取买家秀”。
6. 点击“添加并立即抓取”。
...
4. 点击批量抓取后，同一账号严格按顺序抓取；只有分配到不同账号浏览器的商品才会并行，调度上限为 5 个。
...
同一商品、同一 SKU 每次抓取到的有效价格严格低于监控价时，都会发送机器人提醒；相等或高于监控价不提醒。
...
依次检查：Webhook 是否已保存、测试消息是否成功、自动提醒开关是否开启、SKU 是否设置监控价、当前有效价格是否严格低于监控价。
