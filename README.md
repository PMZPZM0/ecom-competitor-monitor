...
- 账号视角：多账号抓取结果完全隔离，商品卡片可切换查看；主账号视角单独作为监控阈值、价格记录和飞书来源，首选不可用时按 88VIP → 礼金 → 普通能力顺序回退。
...
- 飞书联动：内置官方 `@larksuite/cli`，支持扫码授权、主账号视角价格文档同步和群机器人低价提醒；该视角的有效价格严格低于 SKU 监控价时提醒。
- 运行反馈：抓取、监控、飞书同步和下载任务均显示进行中、完成或失败原因。
- AI 生图：支持正向提示词、排除要求、最多 4 张参考图、1K 标准与 2K/4K 本机增强输出、生成历史、收藏相册、参数复用和非破坏式批注编辑；图片模型和分析模型独立配置。
- 模型配置：提供“稳定生图 / 高速通道 / 自定义配置”，每条通道独立加密保存 Key；支持免付费连接测试和按通道清除 Key，兼容网关缺少单模型接口时自动回退模型列表。
