...
本文面向第一次使用本软件的同事，按顺序完成启动方式选择、账号授权、商品抓取、价格监控、飞书提醒、运营报表分析和团队数据同步。

软件内“使用说明书”常驻在顶部标准字体旁边，可随时打开，不需要离开当前应用。

![软件内使用说明](screenshots/usage-guide.png)

## 团队网页版与桌面端联动

经营罗盘有两种独立使用方式：桌面端适合商品抓价和个人运营工作；团队网页版适合多人共享运营报表与管理店铺权限。两者不是同一套登录资料，也不会共享淘宝账号。

1. 团队管理员在团队网页版创建店铺团队，并在“同步授权码”选择可同步店铺后生成授权码。
2. 同事在桌面端打开“运营数据 > 数据仓库 > 云端团队数据”，输入团队网页地址、授权码和当前电脑名称。
3. 绑定成功后点击“立即同步”，云端仅下发已授权店铺的运营报表；本机上传的报表保留，系统按来源去重。

淘宝登录状态、浏览器资料、SKU 价格证据、飞书连接、模型 Key 和 QwenPaw Agent 全部只在各自电脑保存。管理员撤销设备授权或同事断开云端后，电脑中已有本地数据不会被删除。

团队网页版开源部署说明见 [cloud-data-hub/README.md](../cloud-data-hub/README.md)。
...
1. 打开“商品监控”，在商品工作台右上角点击“新增商品”。
2. 使用“商品链接”时，粘贴淘宝或天猫长链接；软件会自动清理跟踪参数。使用“商品 ID”时，只输入纯数字 ID 并选择淘宝或天猫，地址前缀会自动补全。
3. 选择主监控账号类型；需要 750 主图、详情图或视频时勾选“抓取完整素材”，需要买家秀时再勾选“同时抓取买家秀”。勾选项会分别加入完整素材队列或买家秀队列，不会混入价格任务。
4. 点击“自动采集并本地解析”。软件会在已授权账号浏览器中后台打开商品页，自动收集页面已加载的 HTML、可见优惠文字和价格响应。
5. 软件删除 Cookie、Authorization、Token、签名和账号身份字段，原子保存本地源文件，关闭写入后重新读盘；标题、店铺、型号、SKU 和价格只从读回的数据解析。型号会优先读取商品页面的“型号/产品型号/货号”等基础参数；旧记录首次打开工作台时也会只用已保存的本地证据自动补齐，不会重新访问商品页。
6. 新商品默认“未启用”。先点击卡片右侧“核对”确认 SKU、金额和公式；点击商品名称进入预览，可查看账号视角、素材、买家秀、SKU 监控价和计划。
...
商品工作台的外层卡片只用于快速判断和执行：左侧是商品信息，中间是最低已验证价，右侧按固定顺序提供“核对、定时、飞书、抓取、删除”。窄屏会自动收成图标操作轨，悬停可查看操作名称。

- **核对**：打开每个 SKU 的全部价格通道、证据和计算公式。
- **定时**：只启用或暂停当前商品的自动监控，不会直接修改计划时间。
- **飞书**：把当前已验证的 SKU 快照同步到已配置的飞书。
- **抓取**：立即更新当前商品，不依赖定时计划。
- **删除**：删除商品、该商品的监控规则和本机历史记录，执行前需要确认。

点击商品名称或图片会打开商品预览；完整内容都在预览中：
...
- 顶部可切换账号价格视角；只有标记“监控”的主账号视角会影响监控价和飞书提醒。
- 预览顶部可设置单次或循环计划，并点击“保存计划”；监控价在对应 SKU 输入框失焦时自动保存。
...
商品工作卡右侧的“定时”先决定商品是否进入自动监控。点击商品名称打开预览后，在预览顶部设置两种互斥模式之一，并点击“保存计划”：
...
暂停全局或点击卡片右侧“定时”暂停本商品，都不会删除尚未执行的计划、SKU 监控价和历史记录。单次定时执行成功后会清除已完成时间并暂停本商品，避免重复执行。商品工作卡会显示当前启用状态；任务中心可查看下次执行时间和排队顺序。
...
![商品工作台](screenshots/product-workbench.png)

筛选后的商品可全选本页，再执行批量抓取、批量下载买家秀或批量删除；批量操作会进入同一个任务中心，不会因为切换页面而丢失。
...
1. 打开“运营数据 > 数据仓库 > 报表上传”，可一次选择多份 XLS、XLSX、CSV 等报表。桌面端先选择店铺；团队网页版会默认识别当前团队店铺，也保留手工选择入口。
2. 每份文件会独立显示“读取中、已识别、待校正、导入中、已完成或失败”，并展示报表类型、统计日期、字段和有效行数。处理中不要重复点击；失败项可单独重试，不用重走全部文件。
3. 自动识别只提供初始值。确认前可逐份修改店铺、统计日期以及日报、周报、月报或自定义周期；月报和周报应填写数据实际覆盖的起止日期，不能把下载日当成统计日。
4. 导入后在数据仓按“店铺 > 统计日期 > 报表”查看，最新日期排在前面。可选择整个店铺、整组日期，或展开后只选择某一份报表，再执行批量调整或删除。
5. 商品资料在“数据仓库 > 商品资料”维护。型号和品类按“店铺 + 商品 ID”分别保存；可下载模板、导入表格、手工新增，或分页勾选后批量修改店铺、品类和型号。重复记录、已经被更新的旧版本或失效选择会被拦截。
6. “销售扣除”始终展示完整历史，方便核对曾经剔除的大单。只有店铺和日期命中当前筛选范围的扣除才参与净 GSV、经营 ROI 与推广费率计算，原始报表不会被改写。

### 查看经营数据

- “整店总览”查看净 GSV、推广花费、管理 ROI 和推广费率；点击指标卡可单选或多选经营趋势。通过卡片设置可从支付金额、退款、访客、转化、点击、加购等全部可计算字段新增最多 6 张自定义卡片，自定义卡片同样可以加入趋势。每项趋势独立缩放，不同数量级同时展示时不会被压成直线。
- “类目销售贡献与推广效率”把类目净 GSV、销售占比、推广花费、花费占比和推广费率放在同一行，便于老板直接比较；金额条按当前数据动态缩放，并跟随店铺、日期与报表口径重算。
- “品类 360”支持多选品类并自动求和，展开推广类型后查看每个计划；上方 TOP 10 图同时比较净 GSV、推广花费和费率。
- “商品排行”支持型号、品类和商品 ID 搜索筛选。品类与型号双向联动，选择品类后只显示该品类可用型号；切换筛选时会自动清除无效型号。
- 品类 360 和商品排行的推广计划详情一次只打开一项。点击另一项会关闭上一项，详情在独立抽屉中展示，不会改变表格滚动位置。
- 日期快捷项包括今天、昨天、近 7 日、近 15 日、本周、上周、本月、上月和自定义。自定义起止日期选择完成后点击“确定”，整店总览、品类 360、商品排行和数据仓库会一起刷新。

### 环比规则

指标卡不需要逐张勾选环比类型，系统会按当前日期快捷项自动选择公式，并把结果直接显示在卡片数值旁：

- 今天、昨天或单日范围：当前日对比前一日。
- 本周、上周：当前自然周截至所选结束日，对比上一自然周的相同天数。
- 本月、上月：当月截至所选结束日，对比上月相同天数；月末会对齐上月月末。
- 近 7 日：当前连续 7 天对比紧邻的前 7 天。
- 近 15 日：当前连续 15 天对比紧邻的前 15 天。
- 自定义：当前起止日期对比紧邻的等长前一周期。

上涨显示红色，下降显示绿色，持平显示灰色。计算会同时限制店铺、报表口径以及当前品类/型号/商品筛选；当前期或同期报表不完整时显示“暂无”，不会把缺失数据当成 0。费率类指标同时显示百分点变化和相对变化，金额与数量显示相对百分比。

类目结构中的三项指标按当前筛选范围计算：

- 销售占比 = 类目净 GSV / 当前有效类目净 GSV 合计。
- 花费占比 = 类目推广花费 / 当前有效类目推广花费合计。
- 类目推广费率 = 类目推广花费 / 类目净 GSV。

团队网页版在手机上会自动切换为老板查看布局：顶部保留四个运营页面入口，核心指标两列排列，筛选面板和经营矩阵改为窄屏卡片。无需安装应用即可查看，但上传、删除和团队管理仍按账号权限控制。

经营数据使用以下固定口径：

- 净 GSV = 支付金额 - 成功退款金额。
- 推广类型整体费率 = 推广类型总花费 / 关联商品链接净 GSV。
- 计划费率 = 计划花费 / 该计划推广成交金额。
- 管理 ROI = 净 GSV / 推广花费。

### 同步团队云端报表

1. 团队管理员在团队网页版生成同步授权码，并选择当前设备可读取的店铺。
2. 桌面端打开“运营数据 > 数据仓库 > 云端团队数据”，填写服务器地址、授权码和设备名称后绑定。
3. 点击“立即同步”。云端报表会合并到本地数据仓库，本机手工上传内容保留，重复的云端来源不会反复写入。
4. 同步是只读下发。淘宝账号、Cookie、价格证据、飞书配置、模型 Key、QwenPaw 和本地聊天不会上传。

团队网页版管理员和成员的完整权限、邀请码、设备限制及自部署方法见 [团队网页版说明](../cloud-data-hub/README.md)。
