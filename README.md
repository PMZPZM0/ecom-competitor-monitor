...
- 账号与团队：用户名 + 团队邀请码免验证码注册，旧 QQ 邮箱账号继续兼容登录；显示名称可独立修改，并支持主动退出团队、解散团队和在已加入团队间切换。平台超级管理员不占团队名额，也不会出现在成员列表中。
...
- 桌面端与团队网页版统一费率判定：`≤9.5%` 绿色、`>9.5% 且 ≤11%` 黄色、`>11%` 红色；整店卡片、类目贡献、TOP 图、经营矩阵、推广类型与计划详情全部使用同一规则。
- 品类 360 与商品排行把筛选工具移到汇总和 TOP 图下方、明细表上方；支持品类/型号联动、最多 3 组手动保存的筛选历史、单组删除/全部清空、清除筛选，以及点击外部自动收起。
- 自定义日期与上传日期始终保证开始日期不晚于结束日期；直接修改结束日期查看单日时会同步修正开始日期，减少重复操作。
- 团队网页版注册改为“用户名 + 密码 + 团队邀请码”，旧 QQ 邮箱账号仍可登录；用户可独立修改显示名称，平台超级管理员不占团队名额，也不会出现在团队成员列表中。
- QwenPaw 新增按店铺、日期、报表周期、实体和指标精准查询运营数据的工具，并公开指标口径；AI 创作补齐参考图登记、需求解析、三套提示方案、生成任务重试/取消及 Photoshop 打开/同步回传。
- 桌面开发版与开源团队网页版版本统一为 `1.0.39`，项目说明、团队部署说明、测试和 GitHub Release 同步更新。
- 淘宝/天猫价格链路没有改动，仍严格执行授权浏览器采集、脱敏原子落盘、从本地文件读回后解析，禁止后端直连商品或价格接口。

## v1.0.38

- 品类 360 与商品排行取消重复的大指标卡，环比直接进入每一行经营矩阵。支付金额、退款、净 GSV、推广花费、推广成交、ROI、推广费率及自定义数据均按当前店铺、实体和统计周期独立计算。
- 品类和商品矩阵分别提供环比开关，设置互不影响并自动保存；上涨显示红色、下降显示绿色、无有效同期数据明确显示 `--`，不补零也不跨店铺取数。
- 两个矩阵均可从标准化经营与付费字段中追加最多 8 个数据列，支持列头排序、拖动调整顺序、按钮微调顺序，并在刷新或重新打开后保留各自配置。
- 大数据量表格的筛选、汇总、同期实体索引和行渲染增加缓存，调整指标、排序或环比时不再重复扫描整份报表；手机端继续使用卡片化矩阵与内部横向滚动，避免页面溢出。
- 桌面开发版与开源团队网页版同步采用相同的计算行为和交互规则；`cloud-data-hub` 源码、接口测试、Docker/systemd 部署资料继续完整公开。
- 淘宝/天猫价格链路没有改动，仍严格执行授权浏览器采集、脱敏原子落盘、从本地文件读回后解析，禁止后端直连商品或价格接口。

## v1.0.37

- 整店总览、品类 360 和商品排行的现有指标卡与自定义指标卡统一增加自动环比。今天/昨天按日环比，本周/上周按周环比，本月/上月按月环比，近 7 日、近 15 日和自定义区间分别对比紧邻的等长前一周期；上涨红色、下降绿色、持平灰色，缺少同期报表时明确显示暂无。
- 自定义卡片可从经营表与推广表的全部可计算指标中选择，整店自定义卡片可直接加入多选趋势；每条趋势独立缩放，金额、人数、ROI 和费率同时查看时不会互相压扁。
- 修复跨月相邻日期未进入本地日环比的问题。比较范围现在由当前筛选结束日期生成，并严格按同店铺、同报表口径、同商品或同品类取数，不使用下载日期或其他店铺数据补位。
- 桌面端与团队网页版均支持多文件批量上传，逐份显示识别、校正、导入、失败和重试状态；云端可自动使用当前团队店铺，也可手工调整店铺、统计日期与日/周/月/自定义报表类型。
- 数据仓库统一按“店铺 > 统计日期 > 报表”归档，支持店铺、日期和单份报表三级选择与批量管理；销售扣除历史不再被当前日期筛选隐藏，只有命中当前统计范围的记录参与计算。
- 商品资料新增分页批量选择与批量修改店铺、品类、型号，使用追加版本记录保留变更链；过期选择和重复数据会被拦截，团队网页版提供批量维护接口。
- 整店类目贡献、品类与商品排行继续共用同一计算核心：净 GSV = 支付金额 - 成功退款金额；推广费率 = 推广花费 / 净 GSV。类目销售占比、花费占比、费率和动态对比条跟随当前店铺与日期范围重算。

## v1.0.36

- 桌面端与团队网页版的整店总览新增“类目销售贡献与推广效率”，同屏展示类目净 GSV、销售占比、推广花费、花费占比和推广费率，并直接标明三项公式。
- 品类 360 与商品排行新增跟随日期、店铺和周期筛选的 TOP 10 销售/花费对比；每项同时标注推广费率，老板无需在图表和表格间反复切换。
- 品类与型号筛选改为双向联动：选择品类后只保留该品类有效型号，选择型号后同步收窄品类，并自动清除已失效选项。
- 推广计划详情改为脱离大表格的独立抽屉，一次只查看一项；展开不会重建整张表或跳到其他型号，品类和单品大数据量下操作更稳定。
- 团队网页版补齐专门的手机经营视图，指标、类目结构、筛选、经营矩阵和计划详情在窄屏下重排，不再裁切下拉框或重要数据。
- 修复网页首次数据加载失败时白屏，改为显示具体错误与重试入口；日期、店铺、品类和型号下拉框保持可用并使用当前工作区数据。
- 本地开发源码版同步网页经营结构，继续复用同一运营计算核心，不另建公式；缺少同周期证据时费率保持 `--`，不补造结果。
- 团队网页版继续完整开源，包含账号、团队、店铺权限、邀请码、设备授权、平台管理、报表工作台及 Docker/systemd 自部署配置。
