...
type Props = {
  ...
  onClose: () => void
  onSave: (intervalMinutes: number, monitorStartAt: string) => Promise<void>
}
...
export function MonitorScheduleDialog({ product, monitor, onClose, onSave }: Props) {
  ...
  async function save() {
    ...
    if (!date || !time || Number.isNaN(startAt.getTime())) {
    ...
    setError('')
    try {
      await onSave(intervalMinutes, startAt.toISOString())
    } catch (reason) {
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/45 p-3 backdrop-blur-[1px] sm:p-6" onMouseDown={onClose}>
      <div role="dialog" aria-modal="true" aria-labelledby="schedule-dialog-title" className="flex w-full max-w-xl flex-col overflow-hidden rounded-md bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          <label className="block"><span className="mb-1.5 block text-sm font-medium text-slate-700">重复周期（分钟）</span><input type="number" min={30} max={1440} step={1} value={interval} onChange={(event) => setIntervalValue(event.target.value)} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm text-slate-800 outline-none focus:border-emerald-400" /><span className="mt-1.5 block text-xs text-slate-500">最少 30 分钟，最多 1440 分钟。开始时间已过时，系统会按周期顺延到下一次。</span></label>
