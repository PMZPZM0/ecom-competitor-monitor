...
这是旧版安装包未完成 Apple 签名与公证导致的 Gatekeeper 拦截，不是 M1/M2/M3/M4 架构选错。优先下载最新的 `mac-arm64.dmg`。仅当安装包来自本仓库官方 Release 且 SHA-256 校验一致时，可在把应用拖入“应用程序”后执行以下临时命令：

```bash
xattr -cr "/Applications/电商竞品监控.app"
```

然后重新打开应用。该命令只用于可信旧包；正式发布包应通过 Apple 签名与公证后直接双击打开。
