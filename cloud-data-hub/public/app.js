...
const ENTITY_FILTER_HISTORY_LIMIT = 3;
...
const state = {
  ...
  toast: null, activity: null, modal: '', activePanel: 'overview', warehousePanel: 'upload', authMode: 'login', authDraft: { username: '', password: '', inviteCode: '' }, authFeedback: null, authBusy: '', bootstrapError: '', copiedCode: '', deviceCode: '', selectedTeamId: window.sessionStorage.getItem(SELECTED_TEAM_KEY) || '', teamMenuOpen: false, platformSettings: { allowTeamCreation: true },
  trendMetrics: ['revenue'], entityUi: { category: { keyword: '', categories: [], models: [], sort: 'revenue', direction: 'desc', expanded: '', scrollTop: 0, filterMenu: '', categoryQuery: '', modelQuery: '', historySaveOpen: false, historyDraft: '' }, product: { keyword: '', categories: [], models: [], sort: 'revenue', direction: 'desc', expanded: '', scrollTop: 0, filterMenu: '', categoryQuery: '', modelQuery: '', historySaveOpen: false, historyDraft: '' } },
...
function loginView() {
  ...
  app.innerHTML = `<main class="auth-page"><section class="auth-card">${brandLockup('团队经营决策平台')}<div class="auth-head"><h1>${register ? '使用团队邀请码注册' : '登录工作台'}</h1><p>${register ? '设置登录用户名并填写团队邀请码，注册后会直接进入对应团队。' : '使用用户名或原 QQ 邮箱进入运营数据与数据仓库。'}</p></div><div class="switcher"><button class="${register ? '' : 'active'}" data-auth="login">登录</button><button class="${register ? 'active' : ''}" data-auth="register">邀请码注册</button></div><form id="auth-form" class="stack"><label class="field"><span>${register ? '用户名' : '用户名或原 QQ 邮箱'}</span><input required name="username" autocomplete="username" minlength="${register ? '2' : '1'}" maxlength="${register ? '40' : '80'}" value="${escape(draft.username)}" placeholder="${register ? '2-40 位中文、字母或数字' : '输入用户名或原 QQ 邮箱'}" /></label><label class="field"><span>密码</span><input required type="password" name="password" autocomplete="${register ? 'new-password' : 'current-password'}" minlength="10" value="${escape(draft.password)}" placeholder="至少 10 位" /></label>${register ? `<label class="field"><span>团队邀请码</span><input required name="inviteCode" value="${escape(draft.inviteCode)}" placeholder="XXXX-XXXX-XXXX" /></label>` : ''}${feedback}<button class="btn primary" type="submit" ${submitting ? 'disabled' : ''}>${submitting ? (register ? '注册并加入中...' : '登录中...') : (register ? '注册并加入团队' : '登录')}</button></form></section></main>`;
  ...
  document.querySelector('#auth-form').addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (register) { const result = await api('/api/auth/register', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ username: form.username, inviteCode: form.inviteCode, password: form.password }) }); state.session = result.user; } else { const result = await api('/api/auth/login', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ username: form.username, password: form.password }) }); state.session = result.user; }
      state.authBusy = ''; state.authFeedback = null; state.authDraft = { username: '', password: '', inviteCode: '' };
      openOperationsHome();
      await bootstrap();
...
function managedMetricGrid(panel, coreCards, visibleRows) {
  ...
  const customHtml = customCards.map((card) => { const definition = customMetricDefinition(card.metricId); const value = customMetricValue(visibleRows, card.metricId); const tone = card.metricId === 'feeRate' ? feeRateTone(value) : ['revenue', 'paidBuyers', 'conversionRate', 'paidItems'].includes(card.metricId) ? 'mint' : ['spend', 'promotionRevenue', 'clicks', 'cpc'].includes(card.metricId) ? 'blue' : ['managementRoi', 'roi', 'cartUsers', 'cartItems'].includes(card.metricId) ? 'orange' : card.metricId === 'refundAmount' ? 'purple' : ''; return metric(definition.label, formatCustomMetric(card.metricId, value), definition.description, tone, badges(card.metricId), panel === 'store' ? card.metricId : ''); }).join('');
...
function normalizeOrderedDateRange(start, end, changed = '') {
  if (!start || !end || start <= end) return { start, end };
  return changed === 'end' ? { start: end, end } : { start, end: start };
}
...
function entityFilterHistoryStorageKey(kind) { return `ecom-operations-entity-filter-history-v1:${kind}`; }
function entityFilterSignature(categories) { return uniqueSorted(categories || []).join('\u0001'); }
function entityFilterHistoryLabel(categories) { return categories.length <= 2 ? categories.join('、') : `${categories.slice(0, 2).join('、')}等 ${categories.length} 个`; }
function loadEntityFilterHistory(kind) {
  try {
    const parsed = JSON.parse(localStorage.getItem(entityFilterHistoryStorageKey(kind)) || '[]');
    if (!Array.isArray(parsed)) return [];
    return parsed.map((item) => ({
      id: typeof item?.id === 'string' ? item.id : '',
      name: typeof item?.name === 'string' ? item.name.trim().slice(0, 32) : '',
      categories: Array.isArray(item?.categories) ? uniqueSorted(item.categories).slice(0, 100) : [],
      updatedAt: typeof item?.updatedAt === 'string' ? item.updatedAt : '',
    })).filter((item) => item.id && item.categories.length).slice(0, ENTITY_FILTER_HISTORY_LIMIT);
  } catch { return []; }
}
function saveEntityFilterHistory(kind, items) {
  try { localStorage.setItem(entityFilterHistoryStorageKey(kind), JSON.stringify(items.slice(0, ENTITY_FILTER_HISTORY_LIMIT))); } catch { /* Restricted storage must not block filtering. */ }
}
function upsertEntityFilterHistory(kind, categories, manualName = '') {
  const names = uniqueSorted(categories || []); if (!names.length) return [];
  const current = loadEntityFilterHistory(kind); const signature = entityFilterSignature(names);
  const existing = current.find((item) => entityFilterSignature(item.categories) === signature);
  const entry = { id: existing?.id || `entity-filter-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, name: String(manualName || '').trim().slice(0, 32) || existing?.name || entityFilterHistoryLabel(names), categories: names, updatedAt: new Date().toISOString() };
  const next = [entry, ...current.filter((item) => item.id !== existing?.id)].slice(0, ENTITY_FILTER_HISTORY_LIMIT);
  saveEntityFilterHistory(kind, next); return next;
}
...
function entityFilterHistorySection(kind, chosen) {
  const ui = entityUi(kind); const history = loadEntityFilterHistory(kind);
  const saveEditor = ui.historySaveOpen ? `<div class="entity-filter-history-editor"><input data-entity-filter-history-name="${kind}" value="${escape(ui.historyDraft || '')}" placeholder="给这组类目命名" maxlength="32" /><button data-entity-filter-history-save="${kind}" ${chosen.length ? '' : 'disabled'}>保存</button></div>` : '';
  const saved = history.length ? `<div class="entity-filter-history-list">${history.map((entry) => `<span class="entity-filter-history-chip"><button data-entity-filter-history-apply="${kind}:${escape(entry.id)}" title="应用：${escape(entry.categories.join('、'))}">${escape(entry.name)}</button><button data-entity-filter-history-delete="${kind}:${escape(entry.id)}" title="删除${escape(entry.name)}" aria-label="删除已保存组合${escape(entry.name)}">×</button></span>`).join('')}</div>` : '<p class="entity-filter-history-empty">暂无已保存组合，选好类目后可手动保存。</p>';
  return `<section class="entity-filter-history"><header><span>已保存组合 ${history.length}/${ENTITY_FILTER_HISTORY_LIMIT}</span><div><button data-entity-filter-history-open="${kind}" ${chosen.length ? '' : 'disabled'}>${ui.historySaveOpen ? '取消' : '保存当前'}</button>${history.length ? `<button class="danger" data-entity-filter-history-clear="${kind}">清空历史</button>` : ''}</div></header>${saveEditor}${saved}</section>`;
}
...
function feeRateTone(value) {
  ...
  if (Number(value) <= FEE_RATE_QUALIFIED_MAX) return 'good';
  if (Number(value) <= FEE_RATE_WARNING_MAX) return 'warn';
  return 'high';
}
function feeRateValue(value) { return `<span class="fee-rate ${feeRateTone(value)}">${percent(value)}</span>`; }
function entityComparisonChart(rows, kind) {
  ...
  return `<section class="entity-comparison"><header><div><span>TOP 10 · 净 GSV 排名</span><h4>${label}销售与推广花费</h4></div><div class="entity-chart-legend"><i class="revenue"></i><span>净 GSV</span><i class="spend"></i><span>推广花费</span></div></header><div class="entity-bar-list">${ranked.map((row, index) => {
    ...
    return `<article class="entity-bar-row" title="${escape(row.name || primary)}"><div class="entity-bar-label"><i>${String(index + 1).padStart(2, '0')}</i><span><strong>${escape(primary)}</strong><small>${escape(secondary)}</small></span></div><div class="entity-bar-pair"><div><span>净 GSV</span><b class="entity-bar-track"><i class="revenue" style="--bar-width:${width(row.revenue, maxRevenue)}"></i></b><em>${money(row.revenue)}</em></div><div><span>花费</span><b class="entity-bar-track"><i class="spend" style="--bar-width:${width(row.spend, maxSpend)}"></i></b><em>${money(row.spend)}</em></div></div><div class="entity-bar-rate"><span>推广费率</span>${feeRateValue(rate)}</div></article>`;
...
function promotionDetails(row) {
  ...
  const planMetrics = (plan) => {
    return `<dl class="promotion-plan-metrics"><div><dt>花费</dt><dd>${money(plan.spend)}</dd></div><div><dt>计划成交</dt><dd>${money(plan.promotionRevenue)}</dd></div><div><dt>投产</dt><dd>${fmtNumber(plan.roi)}</dd></div><div class="plan-fee-rate"><dt>计划费率</dt><dd>${feeRateValue(plan.feeRate)}</dd></div></dl>`;
  };
  return `<div class="promotion-grid">${channels.map((channel) => {
    ...
    return `<section class="promotion-channel ${promotionTone(channel.name)}"><header class="promotion-channel-head"><div><strong>${escape(channel.name || '未识别推广类型')}</strong><span>${plans.length} 个计划</span></div><div class="promotion-channel-summary"><span>关联净 GSV <b>${money(channel.linkedRevenue)}</b></span><span class="channel-fee-rate">${escape(channel.name || '推广')}整体费率 ${feeRateValue(channel.feeRate)}</span><span>花费 <b>${money(channel.spend)}</b></span></div></header><div class="promotion-plan-list">${plans.map((plan) => `<article class="promotion-plan"><strong class="promotion-plan-name" title="${escape(plan.name || '未命名计划')}">${escape(plan.name || '未命名计划')}</strong>${planMetrics(plan)}</article>`).join('')}</div></section>`;
  }).join('')}</div>`;
}
...
function overviewPanel(workspace) {
  ...
  const cards = [
    ...
    { id: 'store-fee-rate', metricId: 'feeRate', label: '推广费率', value: percent(verified.feeRate), detail: dashboard.sourceCoverage?.storePromotionComplete ? '推广花费 ÷ 净 GSV' : '需同周期单品付费报表', tone: feeRateTone(verified.feeRate) },
  ];
...
function profileModal() {
  ...
  return `<div class="modal"><section class="modal-card profile-modal"><header class="modal-head"><div><h3>个人资料</h3><p>显示名称用于团队成员列表；登录账号不会随名称修改。</p></div><button class="btn text" data-close-modal>关闭</button></header><form id="profile-form" class="modal-body"><div class="profile-account"><span>登录账号</span><strong>${escape(user.username || '')}</strong><small>${user.role === 'platform-admin' ? '平台超级管理员' : '账号全局唯一，不可修改'}</small></div><label class="field"><span>显示名称</span><input required name="displayName" minlength="1" maxlength="40" autocomplete="name" value="${escape(user.displayName || user.username || '')}" placeholder="输入团队中显示的名称" /></label><div class="modal-actions"><button class="btn primary" type="submit">保存名称</button><button class="btn secondary" type="button" data-close-modal>取消</button></div></form></section></div>`;
}
function teamView() {
...
function enhancedTeamView() {
  ...
  const existingMemberOptions = platformControl ? (state.team.memberDirectory || [])
    ...
    .map((member) => `<option value="${escape(member.id)}">${escape(member.displayName || member.username)} · ${escape(member.username)}</option>`).join('') : '';
...
function shell() {
  ...
  app.innerHTML = `<div class="app-shell">${topNav()}<main class="app-main">${view}</main>${activity}${state.toast ? `<div class="toast ${state.toast.error ? 'error' : ''}"><span>${escape(state.toast.message)}</span><button id="dismiss-toast">关闭</button></div>` : ''}${state.modal === 'upload' ? uploadModal(state.upload.mode) : state.modal === 'platform-team' ? platformTeamModal() : state.modal === 'profile' ? profileModal() : ''}${customCardSettingsModal()}</div>`;
...
function bindShell() {
  ...
  document.querySelector('#open-profile')?.addEventListener('click', () => { state.modal = 'profile'; render(); });
  document.querySelector('#profile-form')?.addEventListener('submit', async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); try { const result = await runActivity('正在保存个人名称', () => api('/api/account/profile', { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ displayName: form.get('displayName') }) })); state.session = result.user; state.modal = ''; setToast(result.message || '个人名称已更新。'); render(); } catch (error) { setToast(error.message, true); } });
  ...
  document.querySelector('#filter-start')?.addEventListener('change', (event) => { const endInput = document.querySelector('#filter-end'); if (!endInput) return; const ordered = normalizeOrderedDateRange(event.currentTarget.value, endInput.value, 'start'); event.currentTarget.value = ordered.start; endInput.value = ordered.end; });
  document.querySelector('#filter-end')?.addEventListener('change', (event) => { const startInput = document.querySelector('#filter-start'); if (!startInput) return; const ordered = normalizeOrderedDateRange(startInput.value, event.currentTarget.value, 'end'); startInput.value = ordered.start; event.currentTarget.value = ordered.end; });
  document.querySelector('#apply-custom-scope')?.addEventListener('click', async () => { const startInput = document.querySelector('#filter-start'); const endInput = document.querySelector('#filter-end'); const ordered = normalizeOrderedDateRange(startInput?.value || '', endInput?.value || '', 'end'); if (!ordered.start || !ordered.end) return setToast('请选择有效的自定义统计日期。', true); state.datePreset = 'custom'; state.customScopeOpen = false; await applyScope(ordered).catch((error) => setToast(error.message, true)); render(); });
  ...
  document.querySelector('[data-upload-date-apply]')?.addEventListener('click', () => { const item = activeUploadItem(); if (!item?.periodStart || !item?.periodEnd) return setToast('请在日历中选择开始和结束日期。', true); const ordered = normalizeOrderedDateRange(item.periodStart, item.periodEnd, 'end'); updateUploadItem(item.id, { periodStart: ordered.start, periodEnd: ordered.end, periodKind: periodKindForRange(ordered.start, ordered.end) }); state.upload.openMenu = ''; refreshUploadSubmitState(); render(); });
  ...
  document.querySelectorAll('[data-entity-filter-history-open]').forEach((button) => button.addEventListener('click', () => { const ui = entityUi(button.dataset.entityFilterHistoryOpen); if (!ui || !ui.categories.length) return; ui.historySaveOpen = !ui.historySaveOpen; if (!ui.historySaveOpen) ui.historyDraft = ''; render(); }));
  document.querySelectorAll('[data-entity-filter-history-name]').forEach((input) => {
    input.addEventListener('input', (event) => { const ui = entityUi(event.currentTarget.dataset.entityFilterHistoryName); if (ui) ui.historyDraft = event.currentTarget.value; });
    input.addEventListener('keydown', (event) => { if (event.key !== 'Enter') return; event.preventDefault(); document.querySelector(`[data-entity-filter-history-save="${CSS.escape(event.currentTarget.dataset.entityFilterHistoryName)}"]`)?.click(); });
  });
  document.querySelectorAll('[data-entity-filter-history-save]').forEach((button) => button.addEventListener('click', () => { const kind = button.dataset.entityFilterHistorySave; const ui = entityUi(kind); if (!ui?.categories.length) return; upsertEntityFilterHistory(kind, ui.categories, ui.historyDraft); ui.historySaveOpen = false; ui.historyDraft = ''; ui.filterMenu = 'category'; setToast('筛选组合已保存，下次可一键恢复。'); render(); }));
  document.querySelectorAll('[data-entity-filter-history-apply]').forEach((button) => button.addEventListener('click', () => { const { kind, key: id } = parseEntityTarget(button.dataset.entityFilterHistoryApply); const ui = entityUi(kind); const entry = loadEntityFilterHistory(kind).find((item) => item.id === id); if (!ui || !entry) return; const allowed = new Set(entityFilterOptions(kind, 'category', entitySourceRows(kind), false)); ui.categories = entry.categories.filter((name) => allowed.has(name)); ui.categoryQuery = ''; normalizeEntityLinkedSelection(kind, 'category'); ui.filterMenu = 'category'; render(); }));
  document.querySelectorAll('[data-entity-filter-history-delete]').forEach((button) => button.addEventListener('click', () => { const { kind, key: id } = parseEntityTarget(button.dataset.entityFilterHistoryDelete); const next = loadEntityFilterHistory(kind).filter((item) => item.id !== id); saveEntityFilterHistory(kind, next); entityUi(kind).filterMenu = 'category'; render(); }));
  document.querySelectorAll('[data-entity-filter-history-clear]').forEach((button) => button.addEventListener('click', () => { const kind = button.dataset.entityFilterHistoryClear; saveEntityFilterHistory(kind, []); const ui = entityUi(kind); ui.historySaveOpen = false; ui.historyDraft = ''; ui.filterMenu = 'category'; render(); }));
...
document.addEventListener('click', (event) => {
  if (event.target instanceof Element && event.target.closest('.entity-filter')) return;
  let changed = false;
  for (const ui of Object.values(state.entityUi)) {
    if (!ui.filterMenu) continue;
    ui.filterMenu = ''; ui.historySaveOpen = false; ui.historyDraft = ''; changed = true;
  }
  if (changed) render();
});
