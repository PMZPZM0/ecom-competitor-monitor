...
import { CheckCircle2, CircleAlert, CloudDownload, ExternalLink, LoaderCircle, RefreshCw, X } from 'lucide-react'
...
export function UpdateDialog({ currentVersion, info, checking, error, onCheck, onClose }: Props) {
  ...
  const assetSize = info?.assetSize ? `${(info.assetSize / 1024 / 1024).toFixed(1)} MB` : ''
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/50 p-3 backdrop-blur-[1px] sm:p-6" onMouseDown={onClose}>
      <div role="dialog" aria-modal="true" aria-labelledby="update-dialog-title" className="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl flex-col overflow-hidden rounded-md bg-white shadow-2xl sm:max-h-[calc(100dvh-3rem)]" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <footer className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 bg-slate-50 px-5 py-3">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2"><Button type="button" variant="secondary" onClick={onClose}>关闭</Button>{info?.updateAvailable && info.acceleratedDownloadUrl && <a href={info.downloadUrl} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center justify-center gap-2 whitespace-nowrap rounded-md border border-slate-300 bg-white px-3 text-sm font-medium text-slate-700 hover:bg-slate-100"><ExternalLink className="h-4 w-4" />GitHub 原地址</a>}{info?.updateAvailable && <a href={info.acceleratedDownloadUrl || info.downloadUrl} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center justify-center gap-2 whitespace-nowrap rounded-md border border-blue-600 bg-blue-600 px-4 text-sm font-medium text-white hover:bg-blue-700"><CloudDownload className="h-4 w-4" />{info.assetName ? info.acceleratedDownloadUrl ? '加速下载' : '下载新版安装包' : '打开 GitHub Releases'}</a>}</div>
        </footer>
      </div>
    </div>,
    document.body,
