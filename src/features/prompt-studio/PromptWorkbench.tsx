      ...
      <div className="grid min-w-0 items-start gap-3 xl:min-h-[calc(100dvh-6rem)] xl:grid-cols-[minmax(240px,0.78fr)_minmax(340px,1.08fr)_minmax(320px,1fr)]">
        ...
        <main className="creative-surface scrollbar-thin min-w-0 overflow-hidden rounded-md border border-white/70 shadow-sm xl:max-h-[calc(100dvh-8rem)] xl:overflow-y-auto" aria-label="结构化创作要求">
          ...
          <div className="grid gap-4 p-4">
            ...
            <section className="border-t border-slate-100 pt-4"><SectionTitle title="推荐输出参数" note="同步到生图页后仍可调整，不会在这里直接产生费用。" /><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4"><label className="grid gap-1.5 text-xs font-medium text-slate-600">画面比例<select value={draft.parameters.ratio} onChange={(event) => setDraft((current) => ({ ...current, parameters: { ...current.parameters, ratio: event.target.value as typeof current.parameters.ratio } }))} className="h-10 rounded-md border border-slate-200 bg-white px-2 text-sm text-slate-800"><option>1:1</option><option>4:5</option><option>3:4</option><option>2:3</option><option>9:16</option><option>4:3</option><option>3:2</option><option>16:9</option></select></label><label className="grid gap-1.5 text-xs font-medium text-slate-600">分辨率<select value={draft.parameters.resolution} onChange={(event) => setDraft((current) => ({ ...current, parameters: { ...current.parameters, resolution: event.target.value as typeof current.parameters.resolution } }))} className="h-10 rounded-md border border-slate-200 bg-white px-2 text-sm text-slate-800"><option value="1k">1K</option><option value="2k">2K</option><option value="4k">4K</option></select></label><label className="grid gap-1.5 text-xs font-medium text-slate-600">质量<select value={draft.parameters.quality} onChange={(event) => setDraft((current) => ({ ...current, parameters: { ...current.parameters, quality: event.target.value as typeof current.parameters.quality } }))} className="h-10 rounded-md border border-slate-200 bg-white px-2 text-sm text-slate-800"><option value="low">快速</option><option value="medium">标准</option><option value="high">精细</option></select></label><label className="grid gap-1.5 text-xs font-medium text-slate-600">背景<select value={draft.parameters.background} onChange={(event) => setDraft((current) => ({ ...current, parameters: { ...current.parameters, background: event.target.value as typeof current.parameters.background } }))} className="h-10 rounded-md border border-slate-200 bg-white px-2 text-sm text-slate-800"><option value="auto">自动</option><option value="opaque">不透明</option><option value="transparent">透明</option></select></label></div></section>
